...
#             transforms.ToTensor(),
#             # transforms.Resize(fundus_img_size[0][0])  #不改变长宽比
#             transforms.Resize([fundus_img_size[0][0],fundus_img_size[0][0]])
...
#         oct_img = self.__resize_oct_data__(oct_img)  #resize
#         fundus_img = fundus_img / 255.0  #normalize
...
#         else:
#             fundus_img = self.fundus_val_transforms(fundus_img.astype(np.float32))
#             oct_img = self.oct_val_transforms(oct_img.astype(np.float32))
#         # data[0] = fundus_img.transpose(2, 0, 1) # H, W, C -> C, H, W
#         # data[1] = oct_img.squeeze(-1) # D, H, W, 1 -> D, H, W
#         data[0] = fundus_img
#         data[1] = oct_img.unsqueeze(0)

#         label = label.argmax()  #返回最大索引值，适用于多分类
...
class GAMMA_sub1_dataset(Dataset):  #暂时固定！
    def __init__(self,
                 dataset_root,
                 oct_img_size,
                 fundus_img_size,
        ...
        self.fundus_train_transforms = transforms.Compose([
            transforms.ToTensor(),
            # transforms.RandomResizedCrop(fundus_img_size[0][0], scale=(0.90, 1.1), ratio=(0.90, 1.1)),
            transforms.Resize([fundus_img_size[0][0],fundus_img_size[0][0]]),
            transforms.RandomApply([
                transforms.ColorJitter(0.2, 0.2, 0.2, 0.1)
            ], p=0.8),
...
class OLIVES_dataset_v1(Dataset):
    """
    ...
    def __init__(self, dataset_root,
        ...
        """
        super(OLIVES_dataset_v1, self).__init__()
        self.dataset_root = dataset_root
        self.input_D = oct_img_size[0][0]
        self.input_H = oct_img_size[0][1]
        self.input_W = oct_img_size[0][2]
        ...
        self.file_list = []
        for f in filelists:
            self.file_list.append([f, label[f]])
    ...
    def __getitem__(self, idx):
        ...
        # Fundus read
        fundus_img_path = os.path.join(self.dataset_root, real_index,real_index +".tif")  #fundus路径
        fundus_img = cv2.imread(fundus_img_path)[:,:,::-1] # ,cv2.IMREAD_UNCHANGED：灰度图像，二维数组
        # OCT read
        oct_series_list = os.listdir(os.path.join(self.dataset_root, real_index, real_index))
        oct_series_0 = cv2.imread(os.path.join(self.dataset_root, real_index, real_index, oct_series_list[0]),
                                  cv2.IMREAD_GRAYSCALE)
...
class OLIVES_dataset_v2(Dataset):
    """
    load multi-view data
    对比学习——corolla
    """
    ...
    def __getitem__(self, idx):
        ...
        oct_img = oct_img / 255.0
        if self.mode == "train":
            fundus_img1 = self.fundus_train_transforms(fundus_img.astype(np.float32))
            oct_img1 = self.oct_train_transforms(oct_img.astype(np.float32))
            fundus_img2 = self.fundus_train_transforms(fundus_img.astype(np.float32))
            oct_img2 = self.oct_train_transforms(oct_img.astype(np.float32))
            data[0] = fundus_img1
            data[1] = fundus_img2
            data[2] = oct_img1.unsqueeze(0)
            data[3] = oct_img1.unsqueeze(0)
        else:
            fundus_img = self.fundus_val_transforms(fundus_img.astype(np.float32))
            oct_img = self.oct_val_transforms(oct_img.astype(np.float32))
            data[0] = fundus_img
